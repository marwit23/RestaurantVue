<template>
    <div>
                ...
                <tr v-for="recipe in paginatedData" :key="recipe.dishId">
                    <td>
                        <b-button
                            ...
                            @click="setTempRecipe(recipe)"
                            class="btn btn-light btn-sm ml-2"
                            >View
                        </b-button>

                        <b-button
                            variant="btn btn-light btn-sm ml-2"
        ...
        >
            ...
            <div v-else>
                <p class="my-4 font-weight-bold">
                    Unable to delete. This recipe is in use!
                </p>
                <p class="my4">
                    All cooking orders that use this dish must be first removed
                    from the list in User Panel.
                </p>
                <div class="float-right">
                    <b-button variant="warning" class="ml-2" @click="onCancel"
                        >OK</b-button
                    >
                </div>
            </div>
        ...
        <b-modal id="modal-2" ok-only okTitle="OK" okVariant="warning">
            <p class="my-4 font-weight-bold">
                Unable to edit. This recipe is in use!
            </p>
            <p class="my4">
                All cooking orders that use this dish must be first removed from
                the list in User Panel.
            </p>
        </b-modal>
...
export default {
    ...
    methods: {
        ...
        setTempRecipe(tempRecipe) {
            ...
            const tempArray2 = tempArray.filter(
                (dishName) => dishName == tempRecipe.dishName
            );
