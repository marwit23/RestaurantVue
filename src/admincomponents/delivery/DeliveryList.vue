<template>
  <div>
    <table class="table smaller-font" style="border-top: hidden">
      ...
      <tbody>
        <tr v-for="delivery in paginatedData" :key="delivery.deliveryId">
          <td>
            <b-button
              v-b-modal.view-delivery-modal
              @click="setTempDelivery(delivery.deliveryId)"
              class="btn btn-light btn-sm ml-2"
              >View
            </b-button>
            <b-button
              v-if="delivery.deliveryStatus == 'ORDERED'"
              v-b-modal.modal-1
              @click="setTempDelivery(delivery.deliveryId)"
            ...
            >
            <router-link
              to="/admin/confirm-delivery"
              v-if="delivery.deliveryStatus == 'ORDERED'"
            >
              <b-button variant="success btn-sm ml-2">
                <span @click="setTempDelivery(delivery.deliveryId)"
                  >Confirm</span
                ></b-button
              >
            </router-link>
    ...
    <b-modal
      ...
      @ok="deleteDelivery(tempDelivery.deliveryId)"
    >
      <h5 class="mb-4">
